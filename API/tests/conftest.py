...
@pytest.fixture()
def temp_user_db_file(tmp_path, monkeypatch):
    temp_file = tmp_path / "users_db_test.json"
    if temp_file.exists():
        temp_file.unlink()
    monkeypatch.setenv("AUTH_USERS_DB_FILE", str(temp_file))
    monkeypatch.setenv("AUTH_SECRET_KEY", "test-secret-key-with-enough-length")
    monkeypatch.setenv("AUTH_TOKEN_EXPIRATION_MINUTES", "60")
    monkeypatch.setenv("AUTH_BOOTSTRAP_EMAIL", "doctor@example.com")
    monkeypatch.setenv("AUTH_BOOTSTRAP_PASSWORD", "StrongPass123!")
    monkeypatch.setenv("AUTH_BOOTSTRAP_NAME", "Dra. Teste")
    monkeypatch.setenv("AUTH_BOOTSTRAP_PROFILES", "administrador,medico,recepcao,usuario")
    return str(temp_file)
...
@pytest.fixture()
def flask_app(stubbed_modules, temp_db_file, temp_user_db_file, env_for_config):
    """
    Import server after stubbing heavy modules. Returns Flask app.
    """
    for module_name in ["backend.config", "backend.auth_service", "backend.auth_decorators", "server"]:
