...
try:
    import jwt as pyjwt
except ImportError:  # pragma: no cover - exercised only when PyJWT is absent locally
    pyjwt = None
...
def get_auth_secret() -> str:
    ...
    raise AuthError("AUTH_SECRET_KEY não configurado", 500)
...
def load_users_database() -> dict[str, Any]:
    ...
    try:
        ...
        print(f"[AUTH] Não foi possível carregar {USERS_DB_FILE}. Usando base vazia.")
        return {"users": []}
...
def create_access_token(user: dict[str, Any], profile: str) -> tuple[str, int]:
    ...
    if normalized_profile not in normalize_profiles(user.get("profiles")):
        raise AuthError("Perfil não permitido para este usuário", 403)
...
def decode_access_token(token: str) -> dict[str, Any]:
    if not token:
        raise AuthError("Token obrigatório", 401)
