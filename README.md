...
4. Configure as variaveis de autenticacao no arquivo `.env`:
```env
AUTH_SECRET_KEY=gere_um_segredo_longo_e_aleatorio
AUTH_TOKEN_EXPIRATION_MINUTES=60
AUTH_USERS_DB_FILE=./backend/users_db.json
AUTH_BOOTSTRAP_EMAIL=admin@example.com
AUTH_BOOTSTRAP_PASSWORD=troque_esta_senha
AUTH_BOOTSTRAP_NAME=Administrador
AUTH_BOOTSTRAP_PROFILES=administrador,medico,recepcao,usuario
CORS_ORIGINS=*
```

`AUTH_BOOTSTRAP_EMAIL` e `AUTH_BOOTSTRAP_PASSWORD` criam o primeiro usuario apenas quando o arquivo de usuarios ainda nao existe. Em producao, use um segredo forte em `AUTH_SECRET_KEY` e restrinja `CORS_ORIGINS` para as origens reais do frontend/extensao.
